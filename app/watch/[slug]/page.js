...
export default async function WatchPage({params}){
  ...
  return <><Header/><main className="wide watchLayout">
    <section className="watchMain"><Player video={video} aspect={aspect}/><div className="watchMeta lockedWatchMeta"><div>{video.premium&&<div className="yidlyPremiumBadge">YIDLY PREMIUM</div>}<h1>{video.title}</h1><div className="meta">{video.client} &nbsp;•&nbsp; {(video.categories?.[0]||video.category)}{video.duration?<> &nbsp;•&nbsp; {video.duration}</>:null} &nbsp;•&nbsp; HD</div></div><ShareButton/></div>
    ...
    <aside className="upNext"><div className="upNextTitle">Up Next</div>{next.map(v=><a className="upNextItem" href={`/watch/${v.slug}`} key={v.slug}><div className="upNextThumb" style={{backgroundImage:`url(${v.thumbnail})`}}>{v.duration&&<span>{v.duration}</span>}</div><div><strong>{v.title}</strong><small>{v.categories?.[0]||v.category}</small></div></a>)}</aside>
  </main><Footer/></>
}
