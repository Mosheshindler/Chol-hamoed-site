...
export default async function WatchPage({params}){
  ...
  return <><Header/><main className="wide watchLayout">
    <section className="watchMain"><Player video={video} aspect={aspect}/><div className="watchMeta lockedWatchMeta"><div>{video.premium&&<div className="yidlyPremiumBadge">YIDLY PREMIUM</div>}<h1>{video.title}</h1><div className="meta">{(video.categories?.[0]||video.category)}{video.duration?<> &nbsp;•&nbsp; {video.duration}</>:null} &nbsp;•&nbsp; HD</div></div><ShareButton/></div>
