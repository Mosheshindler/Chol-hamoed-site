...
const emptyForm={id:null,title:'',slug:'',video_url:'',platform:'vimeo',vimeo_hash:'',client:'Mint Media',category:'Behind the Scenes',categories:['Behind the Scenes'],tags:[],thumbnail_url:'',hero_image_url:'',duration_seconds:'',featured:false,featured_home:false,show_just_minted:true,premium:false,purchase_url:'',published:true,sort_order:0}
...
export default function AdminPage(){
  ...
  const allTags=useMemo(()=>{
    const set=new Set()
    videos.forEach(v=>(Array.isArray(v.tags)?v.tags:[]).forEach(t=>set.add(t)))
  ...
  async function saveVideo(e){
    ...
    try{
      ...
      const payload={
        ...
        client:form.client?.trim()||null,category:(form.categories?.[0]||form.category||null),categories:(form.categories?.length?form.categories:[form.category].filter(Boolean)),tags:(form.tags||[]),thumbnail_url:thumbnail||null,hero_image_url:heroImage,
        duration_seconds:form.duration_seconds===''?null:Number(form.duration_seconds),featured:Boolean(form.featured),featured_home:Boolean(form.featured_home),show_just_minted:Boolean(form.show_just_minted),premium:Boolean(form.premium),purchase_url:form.premium?(form.purchase_url?.trim()||null):null,published:Boolean(form.published),sort_order:Number(form.sort_order)||0
      }
  ...
  function editVideo(v){
    setForm({...emptyForm,...v,categories:(Array.isArray(v.categories)&&v.categories.length?v.categories:[v.category].filter(Boolean)),tags:(Array.isArray(v.tags)?v.tags:[]),duration_seconds:v.duration_seconds??'',show_just_minted:v.show_just_minted!==false});setThumbFile(null);setHeroFile(null);setMessage('Editing video.');window.scrollTo({top:0,behavior:'smooth'})
  }
