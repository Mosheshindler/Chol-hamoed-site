...
// Kept in sync with the site's real category nav (lib/site.js) — this used to be its own
// hardcoded list that had drifted to include old categories no page on the site actually
...
export default function AdminPage(){
  ...
  // Deduplicated case-insensitively (keeping whichever casing was seen first) — different
  // videos have the same tag saved with different casing ("Long Form" vs "long form"), and
  // toggleTag below already treats those as the same tag. Without this, both casings showed
  // up as separate-looking buttons that silently fought each other: clicking one could
  // remove the other instead of adding a new tag, since toggleTag's case-insensitive check
  // saw them as identical.
  const allTags=useMemo(()=>{
    const seen=new Map()
    videos.forEach(v=>(Array.isArray(v.tags)?v.tags:[]).forEach(t=>{
