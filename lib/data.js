...
function mapRow(row){
  return {
    ...
    isYouTubeShort:row.platform==='youtube' && /\/shorts\//.test(row.video_url||''),
...
// Newest-upload-first, using the video's real YouTube/Vimeo publish date (fetched once when
// the video is added/edited — see /api/video-meta) rather than when it was posted to this
// site. Videos without a fetched date (added before this existed, or the lookup failed) sort
// after ones that have it, keeping their existing relative order rather than being scattered
// — a stable sort, so anything already in a sensible order (e.g. the site-wide default) stays
// that way among themselves. This is only ever the *fallback* order for a section — anywhere
// with its own manual reorder (the per-category/Home/Hero drag-and-drop in admin) uses that
// instead, same as before.
export function sortByPublishDate(list){
  return [...list].sort((a,b)=>{
    const ad=a.sourcePublishedAt?new Date(a.sourcePublishedAt).getTime():null
    const bd=b.sourcePublishedAt?new Date(b.sourcePublishedAt).getTime():null
...
export async function getDatabaseVideos(){
  ...
  if(!url || !key) return []
  try{
    // `cache:'no-store'` meant every single page view re-fetched every video from Supabase
    // fresh, on every request — real added latency for every visitor, for data that only
    // ever changes when someone saves something in /admin. A short revalidation window
    // means changes still show up within half a minute, but repeat visits are served from
    // Vercel's cache instead of round-tripping to Supabase each time.
