...
const DEFAULT_SETTINGS={showJustMinted:true}
...
export async function getSiteSettings(){
  ...
  try{
    const res=await fetch(`${url}/rest/v1/site_settings?select=show_just_minted&id=eq.true&limit=1`,{
      headers:{apikey:key,Authorization:`Bearer ${key}`},
      cache:'no-store'
    })
    ...
    return {showJustMinted:rows[0].show_just_minted!==false}
  }catch{
